...
public class EnemySummoner extends AbstractEnemyOrb {
    ...
    public EnemySummoner(int passive)
    {
        this.setID(ID);
    ...
    public void onEvoke()
    {
        ...
        if (this.evokeAmount > 0) { DuelistCard.incMaxSummons(AbstractEnemyDuelist.enemyDuelist, this.evokeAmount); }
        else if (this.evokeAmount < 0) { DuelistCard.decMaxSummons(AbstractEnemyDuelist.enemyDuelist, -this.evokeAmount); }
    }
    ...
    private void triggerPassive()
    {
        ...
        {
            AnyDuelist pa = new AnyDuelist(null, AbstractEnemyDuelist.enemyDuelist);
            if (DuelistCard.getSummons(AbstractEnemyDuelist.enemyDuelist) < DuelistCard.getMaxSummons(AbstractEnemyDuelist.enemyDuelist)) { AbstractDungeon.actionManager.addToBottom(new VFXAction(new OrbFlareEffect(this, OrbFlareEffect.OrbFlareColor.PLASMA), 0.1f)); }
    ...
    {
        if (AbstractEnemyDuelist.enemyDuelist != null && AbstractEnemyDuelist.enemyDuelist.hasPower(FocusPower.POWER_ID))
        {
            if ((AbstractEnemyDuelist.enemyDuelist.getPower(FocusPower.POWER_ID).amount > 0) || (AbstractEnemyDuelist.enemyDuelist.getPower(FocusPower.POWER_ID).amount + this.originalPassive > 0))
            {
                this.basePassiveAmount = this.originalPassive + AbstractEnemyDuelist.enemyDuelist.getPower(FocusPower.POWER_ID).amount;
            }
            ...
            this.baseEvokeAmount = this.originalEvoke + AbstractEnemyDuelist.enemyDuelist.getPower(FocusPower.POWER_ID).amount;
        }
        else
        {
