...
public class Mist extends DuelistOrb {
	...
	public void updateDescription() {
		...
		if (this.passiveAmount > -1) {
			...
			this.description = DESC[3] + -this.passiveAmount + DESC[1] + this.evokeAmount + DESC[2];
		}
	}
	...
	public void onEvoke() {
		...
		applyFocus();
		AbstractCreature target = this.owner.getEnemy() != null ? AbstractDungeon.player : AbstractDungeon.getRandomMonster();
		if (target != null) {
			float eva = this.evokeAmount;
			float amt = DuelistCard.getMaxSummons(this.owner.creature()) / eva;
			int damp = (int) amt;
			this.owner.applyPower(target, this.owner.creature(), new DampDebuff(target, this.owner.creature(), damp));
		}
	}
	...
	public void triggerPassiveEffect() {
		...
		if (this.passiveAmount > 0) {
			...
			DuelistCard.incMaxSummons(this.owner.creature(), this.passiveAmount);
			...
			DuelistCard.decMaxSummons(this.owner.creature(), -this.passiveAmount);
		}
	}
