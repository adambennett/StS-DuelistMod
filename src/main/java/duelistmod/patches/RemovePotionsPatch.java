...
public class RemovePotionsPatch {
    ...
    public static class PotionPatch {
        ...
        public static void Postfix(AbstractPlayer.PlayerClass chosenClass) {
            if (chosenClass == TheDuelistEnum.THE_DUELIST) {
				if (!DuelistMod.persistentDuelistData.GameplaySettings.getOrbPotions()) {
					for (String s : DuelistMod.orbPotionIDs) {
						PotionHelper.potions.remove(s);
						Util.log("Duelist config settings: removing " + s + " from potion pool");
					}
				}
    ...
    public static class PotionAddPatch {
        ...
        public static ArrayList<String> Postfix(ArrayList<String> __result, final AbstractPlayer.PlayerClass c, final boolean getAll) {
            if (c == TheDuelistEnum.THE_DUELIST) {
                ...
                if (Util.deckIs("Naturia Deck") || Util.deckIs("Plant Deck") || Util.deckIs("Insect Deck")) {
                ...
                if (Util.deckIs("Toon Deck") || Util.deckIs("Increment Deck") || Util.deckIs("Beast Deck") || Util.deckIs("Zombie Deck")) {
                    __result.add("BloodPotion");
                }

                // Machine/Dragon/Aqua - Heart of Iron
                if (Util.deckIs("Machine Deck") || Util.deckIs("Dragon Deck") || Util.deckIs("Aqua Deck")) {
                    __result.add("HeartOfIron");
                }

                // Standard/Beast/Creator/Exodia/Increment - Ghost in a Jar
                if (Util.deckIs("Standard Deck") || Util.deckIs("Increment Deck") || Util.deckIs("Beast Deck") || Util.deckIs("Creator Deck") || Util.deckIs("Exodia Deck")) {
                    __result.add("GhostInAJar");
                }

                // Machine/Beast/Standard/Increment - Cunning Potion
                if (Util.deckIs("Machine Deck") || Util.deckIs("Increment Deck") || Util.deckIs("Beast Deck") || Util.deckIs("Standard Deck")) {
                    __result.add("CunningPotion");
                }

                // Spellcaster/Standard/Fiend/Zombie - Essence of Darkness
                if (Util.deckIs("Spellcaster Deck") || Util.deckIs("Standard Deck") || Util.deckIs("Fiend Deck") || Util.deckIs("Zombie Deck")) {
