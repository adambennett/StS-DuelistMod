...
public class EnemyDrawFromTypeAction extends AbstractGameAction
{
	...
	public EnemyDrawFromTypeAction(AnyDuelist duelist, int amount, CardType tag) {
	...
	public void drawTag(int numCards, CardType tag) {
		...
		if (typedCardsFromDeck.size() > 0) {
			for (int i = 0; i < numCards; i++) {
				...
				if ((this.duelist.hasPower("Corruption")) && (c.type == CardType.SKILL)) {
					c.setCostForTurn(-20);
				}
		...
		if (!this.shuffleCheck) {
			...
			if (this.amount > deckSize) {
				...
				if (taggedCardsInDiscard > 0) {
					AbstractDungeon.actionManager.addToTop(new EnemyDrawFromTypeAction(this.duelist, tmp, typeToDraw));
					AbstractDungeon.actionManager.addToTop(new EnemyShuffleOnlyTypedAction(typeToDraw, this.duelist));
				}
				if (deckSize != 0) {
					AbstractDungeon.actionManager.addToTop(new EnemyDrawFromTypeAction(this.duelist, deckSize, typeToDraw));
				}
				this.amount = 0;
				this.isDone = true;
			}
