...
public class EnemyDrawFromRarityAction extends AbstractGameAction {
	...
	public EnemyDrawFromRarityAction(AnyDuelist duelist, int amount, CardRarity tag) {
	...
	public void update()
	{
		...
		if (!this.shuffleCheck) {
			if (this.amount + this.duelist.hand().size() > BaseMod.MAX_HAND_SIZE) {
				int handSizeAndDraw = 10 - (this.amount + this.duelist.hand().size());
			...
			if (this.amount > deckSize) {
				...
				if (taggedCardsInDiscard > 0) {
					AbstractDungeon.actionManager.addToTop(new EnemyDrawFromRarityAction(this.duelist, tmp, tagToDraw));
					AbstractDungeon.actionManager.addToTop(new EnemyShuffleOnlyRarityAction(tagToDraw, this.duelist));
				}
				if (deckSize != 0) {
					AbstractDungeon.actionManager.addToTop(new EnemyDrawFromRarityAction(this.duelist, deckSize, tagToDraw));
				}
				this.amount = 0;
				this.isDone = true;
			}
