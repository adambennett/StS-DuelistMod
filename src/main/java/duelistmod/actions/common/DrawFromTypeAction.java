...
public class DrawFromTypeAction extends AbstractGameAction
{
	private boolean shuffleCheck = false;
	private CardType typeToDraw;

	public DrawFromTypeAction(AbstractCreature source, int amount, boolean endTurnDraw, CardType tag) {
		if (endTurnDraw) {
	...
	public DrawFromTypeAction(AbstractCreature source, int amount, CardType tag) {
		this(source, amount, false, tag);
	}

	public void drawTag(int numCards, CardType tag)
	{
		ArrayList<AbstractCard> typedCardsFromDeck = new ArrayList<AbstractCard>();
		for (AbstractCard c : AbstractDungeon.player.drawPile.group)
		{
			if (c.type.equals(tag))
			{
	...
	public void drawTag(CardType tag)
	{
	...
	public void update()
	{
		...
		if (!this.shuffleCheck) {
			...
			if (this.amount > deckSize) {
				...
				{
					AbstractDungeon.actionManager.addToTop(new DrawFromTypeAction(AbstractDungeon.player, tmp, typeToDraw));
					AbstractDungeon.actionManager.addToTop(new ShuffleOnlyTypedAction(typeToDraw));
					//AbstractDungeon.actionManager.addToTop(new EmptyDeckShuffleAction());
				}
				if (deckSize != 0) {
					AbstractDungeon.actionManager.addToTop(new DrawFromTypeAction(AbstractDungeon.player, deckSize, typeToDraw));
				}
				this.amount = 0;
				this.isDone = true;
			}
