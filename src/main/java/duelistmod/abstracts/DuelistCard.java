...
import static duelistmod.variables.Tags.ZOMBIE;
...
public abstract class DuelistCard extends CustomCard implements CustomSavable <String>
{
	 ...
	 * Static Setup						// Hack together the orb list before the dungeon is loaded
	 * Void Methods						// Empty method calls that can be implemented by Duelist cards
	 * Config Menu Functions			// Functions to handle card settings in the Duelist configurations menu
	...
	public void onSynergyTributeWhileInHand(List<CardTags> allMatchingTypes) { }

	public void onSynergyTributeWhileInDiscard(List<CardTags> allMatchingTypes) { }

	public void onSynergyTributeWhileInExhaust(List<CardTags> allMatchingTypes) { }

	public void onSynergyTributeWhileInDraw(List<CardTags> allMatchingTypes) { }

	public void onSynergyTributeWhileInGraveyard(List<CardTags> allMatchingTypes) { }

	public void onSynergyTributeWhileSummoned(List<CardTags> allMatchingTypes) { }
	...
	protected static String getDesc(String desc, String cardID) {
	...
	@Override
	public void hover() {
		EnemyDuelistCard ac = AbstractEnemyDuelist.fromCardOrNull(this);
		if (ac == null) {
			super.hover();
			return;
		}
	...
	private void handleOnSynergyForAllAbstracts(List<CardTags> allMatchingTypes)
	{
		AnyDuelist p = AnyDuelist.from(this);
		for (AbstractRelic r : p.relics()) { if (r instanceof DuelistRelic) { ((DuelistRelic)r).onSynergyTribute(allMatchingTypes); }}
		for (AbstractOrb o : p.orbs()) { if (o instanceof DuelistOrb) {  ((DuelistOrb)o).onSynergyTribute(allMatchingTypes); }}
		for (AbstractPower pow : p.powers()) { if (pow instanceof DuelistPower) { ((DuelistPower)pow).onSynergyTribute(allMatchingTypes); }}
		for (AbstractCard c : p.hand()) { if (c instanceof DuelistCard) { ((DuelistCard)c).onSynergyTributeWhileInHand(allMatchingTypes); }}
		for (AbstractCard c : p.discardPile()) { if (c instanceof DuelistCard) { ((DuelistCard)c).onSynergyTributeWhileInDiscard(allMatchingTypes); }}
		for (AbstractCard c : p.drawPile()) { if (c instanceof DuelistCard) { ((DuelistCard)c).onSynergyTributeWhileInDraw(allMatchingTypes); }}
		for (AbstractCard c : p.exhaustPile()) { if (c instanceof DuelistCard) { ((DuelistCard)c).onSynergyTributeWhileInExhaust(allMatchingTypes); }}
		for (AbstractCard c : p.resummonPile()) { if (c instanceof DuelistCard) { ((DuelistCard)c).onSynergyTributeWhileInGraveyard(allMatchingTypes); }}
		if (p.hasPower(SummonPower.POWER_ID)) {
			SummonPower pow = (SummonPower)p.getPower(SummonPower.POWER_ID);
			for (DuelistCard c : pow.getCardsSummoned()) {
				c.onSynergyTributeWhileSummoned(allMatchingTypes);
			}
		}
		if (p.player()) {
			for (AbstractPotion pot : p.getPlayer().potions) { if (pot instanceof DuelistPotion) { ((DuelistPotion)pot).onSynergyTribute(allMatchingTypes); }}
		}
		if (p.stance() instanceof DuelistStance) { ((DuelistStance)p.stance()).onSynergyTribute(allMatchingTypes); }
	}
	...
	public static void gainGold(int amount, AbstractCreature source, boolean rain)
	{
		if (!AbstractDungeon.player.hasRelic(Ectoplasm.ID))
		{
			CardCrawlGame.sound.play("GOLD_GAIN");
			AbstractDungeon.actionManager.addToBottom(new ObtainGoldAction(amount, source, rain));
		}
	}
		...
		if (!duelist.hasPower(DepoweredPower.POWER_ID)) {
			...
			// For any non-megatyped monster tributes, just loop through the monster types that the tributed card has to see if any match the tributing card
				...
				for (CardTags t : cardTypes) {
					List<MonsterType> matchingTypes = Arrays.stream(MonsterType.values()).filter(ty -> ty.tag() == t).collect(Collectors.toList());
					if (!matchingTypes.isEmpty()) {
						switch (matchingTypes.get(0)) {
							case AQUA:
	...
	public void synergyTributeOneTimeChecks(DuelistCard tributingCard, DuelistCard tributedCard) {
		...
		ArrayList<CardTags> allMatches = new ArrayList<>();
		...
		if (oneMatchingType) {
			handleOnSynergyForAllAbstracts(allMatches);
	...
	public void warriorSynTrib(DuelistCard tributingCard, AnyDuelist duelist) {
		if (tributingCard.hasTag(Tags.WARRIOR)) {
			...
			if (enable) {
				int blockAmount = DuelistMod.getMonsterSetting(MonsterType.WARRIOR, MonsterType.warriorBlockTributeEffectKey, MonsterType.warriorDefaultTributeBlockAmount);
				if (duelist.hasPower(DexterityPower.POWER_ID)) {
					blockAmount += duelist.getPower(DexterityPower.POWER_ID).amount;
				}
				if (blockAmount > 0) {
					duelist.block(blockAmount);
				}
			}
	...
	public int fiendSynTrib(DuelistCard tributingCard, AnyDuelist duelist) {
		if (tributingCard.hasTag(Tags.FIEND)) {
			...
			int draw = DuelistMod.getMonsterSetting(MonsterType.FIEND, MonsterType.fiendCardsKey, MonsterType.fiendDefaultCards);
	...
	public void plantSynTrib(DuelistCard tributingCard, AnyDuelist duelist) {
		if (tributingCard.hasTag(Tags.PLANT)) {
			...
			if (duelist.hasPower(VioletCrystalPower.POWER_ID)) {
				TwoAmountPower pow = (TwoAmountPower) duelist.getPower(VioletCrystalPower.POWER_ID);
				constrict += pow.amount2;
			}
			if (duelist.getEnemy() != null) {
				constrictAllEnemies(duelist, constrict);
			} else {
				AbstractCreature randomTarget = AbstractDungeon.getRandomMonster();
				if (Util.getChallengeLevel() > 3 && Util.deckIs("Plant Deck")) {
					int roll = AbstractDungeon.cardRandomRng.random(100);
					if (roll <= 5) {
						randomTarget = AbstractDungeon.player;
					}
				}
				if (randomTarget != null) {
					applyPower(new ConstrictedPower(randomTarget, duelist.creature(), constrict), randomTarget);
				}
			}
		}
	}
	...
	public void predaplantSynTrib(DuelistCard tributingCard, AnyDuelist duelist) {
		if (tributingCard.hasTag(Tags.PREDAPLANT)) {
			...
			duelist.applyPowerToSelf(new ThornsPower(duelist.creature(), thorns));
		}
	}
	...
	public void insectSynTrib(DuelistCard tributingCard, AnyDuelist duelist) {
		if (tributingCard.hasTag(Tags.INSECT)) {
			...
			boolean hasBugToken = duelist.relics().stream().anyMatch(r -> r instanceof InsectRelic);
			if (duelist.getEnemy() != null) {
				AbstractPower power = new PoisonPower(AbstractDungeon.player, duelist.getEnemy(), poison);
				addToBot(new ApplyPowerAction(AbstractDungeon.player, duelist.getEnemy(), power, power.amount));
				...
				if (hasBugToken) {
					...
					if (randomTarget != null) {
						applyPower(new PoisonPower(randomTarget, AbstractDungeon.player, poison), randomTarget);
					}
				}
			}
		}
	...
	public void zombieSynTrib(DuelistCard tributingCard, AnyDuelist duelist) {
		if (tributingCard.hasTag(Tags.ZOMBIE) && duelist.player()) {
			int souls = DuelistMod.getMonsterSetting(MonsterType.ZOMBIE, MonsterType.zombieSoulsKey, MonsterType.zombieDefaultSouls);
			Util.modifySouls(souls);
		}
	}
	// =============== /TRIBUTE SYNERGY FUNCTIONS/ =======================================================================================================================================================
	...
	public static void incMaxSummons(AbstractCreature target, int amount)
	{
		...
		if (player != null && incremented && Util.getChallengeLevel() > 3 && (Util.deckIs("Increment Deck"))) {
			damageSelfNotHP(1);
		}
	...
	public static boolean checkForUpgradeResummonAnyDuelist(AbstractCard resummonedCard, AnyDuelist p) {
		boolean amtInc = true;
		for (AbstractPotion pot : p.potions()) { if (pot instanceof DuelistPotion) { amtInc = ((DuelistPotion) pot).upgradeResummon(resummonedCard); if (!amtInc) { return false; }}}
		for (AbstractRelic r : p.relics()) { if (r instanceof DuelistRelic) { amtInc = ((DuelistRelic)r).upgradeResummon(resummonedCard); if (!amtInc) { return false; }}}
		for (AbstractOrb o : p.orbs()) { if (o instanceof DuelistOrb) {  amtInc = ((DuelistOrb)o).upgradeResummon(resummonedCard); if (!amtInc) { return false; }}}
		for (AbstractPower pow : p.powers()) { if (pow instanceof DuelistPower) { amtInc = ((DuelistPower)pow).upgradeResummon(resummonedCard); if (!amtInc) { return false; }}}
		for (AbstractCard c : p.hand()) { if (c instanceof DuelistCard) { amtInc = ((DuelistCard)c).upgradeResummonWhileInHand(resummonedCard); if (!amtInc) { return false; }}}
		for (AbstractCard c : p.discardPile()) { if (c instanceof DuelistCard) { amtInc = ((DuelistCard)c).upgradeResummonWhileInDiscard(resummonedCard); if (!amtInc) { return false; }}}
		for (AbstractCard c : p.drawPile()) { if (c instanceof DuelistCard) { amtInc = ((DuelistCard)c).upgradeResummonWhileInDraw(resummonedCard); if (!amtInc) { return false; }}}
		for (AbstractCard c : p.exhaustPile()) { if (c instanceof DuelistCard) { amtInc = ((DuelistCard)c).upgradeResummonWhileInExhaust(resummonedCard); if (!amtInc) { return false; }}}
		for (AbstractCard c : p.resummonPile()) { if (c instanceof DuelistCard) { amtInc = ((DuelistCard)c).upgradeResummonWhileInGraveyard(resummonedCard); if (!amtInc) { return false; }}}
		if (p.hasPower(SummonPower.POWER_ID)) {
			SummonPower pow = (SummonPower)p.getPower(SummonPower.POWER_ID);
			for (DuelistCard c : pow.getCardsSummoned()) {
				amtInc = c.upgradeResummonWhileSummoned(resummonedCard);
				if (!amtInc) { return false; }
			}
		}
		return amtInc;
	}
	...
	public static boolean checkForUpgradeResummon(AbstractCard resummonedCard)
	{
		...
		for (AbstractOrb o : p.orbs) { if (o instanceof DuelistOrb) {  amtInc = ((DuelistOrb)o).upgradeResummon(resummonedCard); if (!amtInc) { return false; }}}
		for (AbstractPower pow : p.powers) { if (pow instanceof DuelistPower) { amtInc = ((DuelistPower)pow).upgradeResummon(resummonedCard); if (!amtInc) { return false; }}}
		for (AbstractCard c : p.hand.group) { if (c instanceof DuelistCard) { amtInc = ((DuelistCard)c).upgradeResummonWhileInHand(resummonedCard); if (!amtInc) { return false; }}}
		for (AbstractCard c : p.discardPile.group) { if (c instanceof DuelistCard) { amtInc = ((DuelistCard)c).upgradeResummonWhileInDiscard(resummonedCard); if (!amtInc) { return false; }}}
		for (AbstractCard c : p.drawPile.group) { if (c instanceof DuelistCard) { amtInc = ((DuelistCard)c).upgradeResummonWhileInDraw(resummonedCard); if (!amtInc) { return false; }}}
		for (AbstractCard c : p.exhaustPile.group) { if (c instanceof DuelistCard) { amtInc = ((DuelistCard)c).upgradeResummonWhileInExhaust(resummonedCard); if (!amtInc) { return false; }}}
		for (AbstractCard c : TheDuelist.resummonPile.group) { if (c instanceof DuelistCard) { amtInc = ((DuelistCard)c).upgradeResummonWhileInGraveyard(resummonedCard); if (!amtInc) { return false; }}}
		if (player().hasPower(SummonPower.POWER_ID)) {
			SummonPower pow = (SummonPower)player().getPower(SummonPower.POWER_ID);
			for (DuelistCard c : pow.getCardsSummoned()) {
				amtInc = c.upgradeResummonWhileSummoned(resummonedCard);
	...
	public static void anyDuelistResummon(AnyDuelist duelist, AbstractCard cardToResummon, AbstractCreature target, int amtToResummon, boolean upgradeResummonedCard, boolean allowExempt) {
		...
		for (int i = 0; i < amtToResummon; i++) {
			...
			if (duelist.getEnemy() != null && AbstractEnemyDuelist.fromCardOrNull(tmp) != null) {
				EnemyDuelistCard enemyCard = AbstractEnemyDuelist.fromCard(tmp);
	...
	public static void invertAll(int amount)
	{
		...
		int currOrbs = AbstractDungeon.player.filledOrbCount();
		for (int i = 0; i < currOrbs; i++) {
			invert(amount);
		}
	}
	...
	{
		AbstractDungeon.actionManager.addToBottom(new InvertOrbAction(amount));
	}
