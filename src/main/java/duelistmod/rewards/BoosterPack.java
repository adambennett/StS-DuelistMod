...
import duelistmod.relics.BoosterPackHybridEgg;
import duelistmod.relics.BoosterPackMonsterEgg;
import duelistmod.relics.BoosterPackSpellEgg;
import duelistmod.relics.BoosterPackTrapEgg;
...
public abstract class BoosterPack extends CustomReward implements CustomSavable<String> {
	...
	public enum PackRarity { COMMON, UNCOMMON, RARE, SUPER_RARE, SPECIAL }
	...
	public void checkRelics() {
		switch (rarity) {
			case COMMON:
				// ex: if (p.hasRelic(iii.ID)) { this.alwaysUpgrade = true; }
				this.textColor = Settings.CREAM_COLOR;
				break;
			case UNCOMMON:
				this.textColor = Settings.BLUE_TEXT_COLOR;
				break;
			case RARE:
		...
		if (AbstractDungeon.player != null && AbstractDungeon.player.hasRelic(BoosterPackHybridEgg.ID) && rarity.equals(PackRarity.COMMON)) {
	...
	public String onSave() {
		return cardsInPack.stream().map(c -> c.cardID).collect(Collectors.joining("~"));
	}
	...
	public boolean claimReward() {
		...
		//{
			...
			if (AbstractDungeon.player.hasRelic(BoosterPackMonsterEgg.ID)) {
				AbstractDungeon.player.getRelic(BoosterPackMonsterEgg.ID).flash();
			}
	...
	protected boolean upgradeCheck() {
		...
		if (Util.getChallengeLevel() > -1) {
			...
			if (act <= 3) {
				...
				{
                    case 2:
                        return upgradeRoll <= 8;
					case 3:
                        return upgradeRoll <= 14;
					default:
						return false;
				}
			}
			...
			if (act <= 3) {
				switch (act) {
                    case 2:
                        return upgradeRoll <= 12;
					case 3:
                        return upgradeRoll <= 20;
					default:
						return false;
				}
			}
	...
	protected boolean additionalUpgradeCheck(AbstractCard c) {
		if (!(this instanceof OrbPack) && ((c instanceof DuelistCard || c instanceof SearingBlow))) {
			int upgradeRoll = AbstractDungeon.cardRandomRng.random(1, 105);
			if (Util.getChallengeLevel() > -1) {
				int act = AbstractDungeon.actNum;
				if (act <= 3) {
					if (act == 3) {
						return upgradeRoll <= 10;
					}
					return false;
				}
				else {
	...
	public ArrayList<AbstractCard> findAllCards(CardType type, int amt, ArrayList<AbstractCard> cardsSoFar) {
		return findAllCards(null, type, null, null, amt, cardsSoFar, null, null, null, null, null, null, null);
	}
	...
	public ArrayList<AbstractCard> findAllCards(CardType type, int amt, ArrayList<AbstractCard> cardsSoFar, CardRarity excludeRarity) {
		return findAllCards(null, type, null, null, amt, cardsSoFar, excludeRarity, null, null, null, null, null, null);
	}
	...
	public ArrayList<AbstractCard> findAllCards(CardType type, int amt, ArrayList<AbstractCard> cardsSoFar, CardRarity excludeRarity, CardRarity excludeRarityB) {
		return findAllCards(null, type, null, null, amt, cardsSoFar, excludeRarity, excludeRarityB, null, null, null, null, null);
	}

	public ArrayList<AbstractCard> findAllCards(CardRarity rare, int amt, ArrayList<AbstractCard> cardsSoFar) {
		return findAllCards(null, null, null, rare, amt, cardsSoFar, null, null, null, null, null, null, null);
	}

	public ArrayList<AbstractCard> findAllCards(CardTags tag, int amt, ArrayList<AbstractCard> cardsSoFar) {
		return findAllCards(null, null, tag, null, amt, cardsSoFar, null, null, null, null, null, null, null);
	}

	public ArrayList<AbstractCard> findAllCards(Predicate<AbstractCard> condition, int amt, ArrayList<AbstractCard> cardsSoFar) {
		return findAllCards(null, null, null, null, amt, cardsSoFar, null, null, null, null, null, null, condition);
	}

	public ArrayList<AbstractCard> findAllCards(CardTags tag, CardRarity rare, int amt, ArrayList<AbstractCard> cardsSoFar) {
		return findAllCards(null, null, tag, rare, amt, cardsSoFar, null, null, null, null, null, null, null);
	}

	public ArrayList<AbstractCard> findAllCards(CardTags tag, CardType type, int amt, ArrayList<AbstractCard> cardsSoFar) {
		return findAllCards(null, type, tag, null, amt, cardsSoFar, null, null, null, null, null, null, null);
	}
	...
	public ArrayList<AbstractCard> findAllCards(CardTags tag, CardType type, int amt, ArrayList<AbstractCard> cardsSoFar, CardRarity excludeRarity) {
		return findAllCards(null, type, tag, null, amt, cardsSoFar, excludeRarity, null, null, null, null, null, null);
	}
	...
	public ArrayList<AbstractCard> findAllCards(CardTags tag, CardType type, int amt, ArrayList<AbstractCard> cardsSoFar, CardRarity excludeRarity, CardRarity excludeRarityB) {
		return findAllCards(null, type, tag, null, amt, cardsSoFar, excludeRarity, excludeRarityB, null, null, null, null, null);
	}
	...
	public ArrayList<AbstractCard> findAllCards(CardTags tag, int amt, ArrayList<AbstractCard> cardsSoFar, CardRarity excludeRarity) {
		return findAllCards(null, null, tag, null, amt, cardsSoFar, excludeRarity, null, null, null, null, null, null);
	}

	public ArrayList<AbstractCard> findAllCards(Predicate<AbstractCard> condition, int amt, ArrayList<AbstractCard> cardsSoFar, CardRarity excludeRarity) {
		return findAllCards(null, null, null, null, amt, cardsSoFar, excludeRarity, null, null, null, null, null, condition);
	}
	...
	public ArrayList<AbstractCard> findAllCards(CardTags tag, int amt, ArrayList<AbstractCard> cardsSoFar, CardRarity excludeRarity, CardRarity excludeRarityB) {
		return findAllCards(null, null, tag, null, amt, cardsSoFar, excludeRarity, excludeRarityB, null, null, null, null, null);
	}

	public ArrayList<AbstractCard> findAllCards(ArrayList<AbstractCard> toFindFrom, int amt, ArrayList<AbstractCard> cardsSoFar) {
		return findAllCards(toFindFrom, null, null, null, amt, cardsSoFar, null, null, null, null, null, null, null);
	}

	public ArrayList<AbstractCard> findAllCards(int amt, ArrayList<AbstractCard> cardsSoFar, CardRarity excludeRarity) {
		return findAllCards(null, null, null, null, amt, cardsSoFar, excludeRarity, null, null, null, null, null, null);
	}
	...
	public ArrayList<AbstractCard> findAllCards(ArrayList<AbstractCard> toFindFrom, int amt, ArrayList<AbstractCard> cardsSoFar, CardRarity excludeRarity) {
		return findAllCards(toFindFrom, null, null, null, amt, cardsSoFar, excludeRarity, null, null, null, null, null, null);
	}

	@SuppressWarnings("ConstantValue")
    public ArrayList<AbstractCard> findAllCards(ArrayList<AbstractCard> toFindFrom, CardType type, CardTags tag, CardRarity rarity, int amt, ArrayList<AbstractCard> cardsSoFar, CardRarity excludeRarityA, CardRarity excludeRarityB, CardTags excludeTagA, CardTags excludeTagB, CardType excludeTypeA, CardType excludeTypeB, Predicate<AbstractCard> extraCardChecks) {
		if (!allowBasics && onlyBasics) {
			allowBasics = true;
		}
		ArrayList<AbstractCard> toRet = new ArrayList<>();
		HashSet<String> cardsAlreadyInBooster = new HashSet<>();
		for (AbstractCard c : cardsSoFar) {
			cardsAlreadyInBooster.add(c.cardID);
		}

		Predicate<AbstractCard> packCheck = c -> {
			if (cardsAlreadyInBooster.contains(c.cardID)) return false;
			boolean allowRare = rarity == null || c.rarity.equals(rarity);
			if (!allowRare) return false;
			...
			if (!megaTypeFilter) return false;
			...
			if (!secondaryTag) return false;

			boolean predicate = extraCardChecks == null || extraCardChecks.test(c);
			if (!predicate) return false;

			if (c instanceof DuelistCard) {
				DuelistCard dc = (DuelistCard)c;
				return dc.canSpawnInBooster(this);
			}
			return true;
		};
		Runnable removeCards = () -> {
			while (toRet.size() > amt) {
				toRet.remove(AbstractDungeon.cardRandomRng.random(toRet.size() - 1));
			}
		};

		if (toFindFrom != null && !toFindFrom.isEmpty()) {
			toFindFrom.stream().filter(packCheck).forEach(c -> toRet.add(c.makeStatEquivalentCopy()));
			removeCards.run();
		} else {
			if (!onlyBasics) {
				TheDuelist.cardPool.group.stream().filter(packCheck).forEach(c -> toRet.add(c.makeStatEquivalentCopy()));
				removeCards.run();
			}
			if (toRet.size() < amt && allowBasics) {
				DuelistMod.duelColorlessCards.stream().filter(packCheck).forEach(c -> toRet.add(c.makeStatEquivalentCopy()));
				removeCards.run();
			}
		}
		return toRet;
