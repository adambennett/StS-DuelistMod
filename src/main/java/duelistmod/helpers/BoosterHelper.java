...
public class BoosterHelper 
{
	...
	public static void modifyPackSize(int add) {
		...
		if (actualPackSize > maxPackSize) { packSize = maxPackSize; } else if (actualPackSize < 1) { packSize = 1; }
	...
	public static LinkedReward getLinked(BoosterPack pack, boolean eliteVictory, boolean bossVictory) {
		...
		if (roll < 60) {
			...
			if (tierRoll == 1) { return new LinkedReward(pack, RelicTier.COMMON); } else if (tierRoll < 25) { return new LinkedReward(pack, RelicTier.UNCOMMON); } else { return new LinkedReward(pack, RelicTier.RARE); }
		} else {
			...
			if (rareRoll == 1) { return new LinkedReward(pack, PotionRarity.COMMON); } else if (rareRoll < 15) { return new LinkedReward(pack, PotionRarity.UNCOMMON); } else { return new LinkedReward(pack, PotionRarity.RARE); }
		}
	}
	...
	public static void refreshPool(ArrayList<BoosterPack> with) {
		packPool = with == null || with.size() < 1 ? initPackPool() : with;
		if (AbstractDungeon.getCurrMapNode() != null && AbstractDungeon.player != null) {
			if (AbstractDungeon.player.hasRelic(BoosterPackPoolRelic.ID)) {
				((BoosterPackPoolRelic)AbstractDungeon.player.getRelic(BoosterPackPoolRelic.ID)).refreshPool();
			}
		}
	}
	...
	public static ArrayList<BoosterPack> initPackPool() {
		...
		boolean addSpecialRelicPacks = AbstractDungeon.player != null && AbstractDungeon.player.hasRelic(BoosterExtraAllRaresRelic.ID);
