...
public class FusionFire extends DuelistCard 
{
    // TEXT DECLARATION
    ...
    public void use(AbstractPlayer p, AbstractMonster m) 
    {
    	...
    	for (AbstractCard c : p.hand.group) { if (c.hasTag(Tags.MONSTER)) { mons.add(c); }}
    	AbstractMonster targ = null;
    	if (upgraded) { targ = m; }
    	else
    	{
    		AbstractMonster rand = AbstractDungeon.getRandomMonster();
    		if (rand != null) { targ = rand; }
    	}
    	if (targ != null) { this.addToBot(new CardSelectScreenResummonAction(mons, 1, targ)); }
    }

    // Which card to return when making a copy of this card.
    @Override
    public AbstractCard makeCopy() {
        return new FusionFire();
    }
