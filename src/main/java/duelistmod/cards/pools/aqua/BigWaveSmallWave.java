...
public class BigWaveSmallWave extends DuelistCard {
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
    	...
    	this.resummonEffect(tributes, m);
    }

	public void resummonEffect(int tributes, AbstractMonster m) {
		if (m == null) {
			List<AbstractMonster> mons = AbstractDungeon.getMonsters().monsters.stream().filter(mon -> mon != null && !mon.isDead && !mon.isDying && !mon.isDeadOrEscaped() && !mon.isEscaping).collect(Collectors.toList());
			if (mons.isEmpty()) {
				return;
			}

			m = mons.get(AbstractDungeon.cardRandomRng.random(mons.size() - 1));
			if (m == null) {
				return;
			}
		}
		if (tributes > 0) {
			...
			for (DuelistCard c : DuelistMod.myCards) {
				...
				if (c.hasTag(Tags.MONSTER) && !c.hasTag(Tags.NOT_ADDED) && allowResummonsWithExtraChecks(c)) {
					...
					if (c.isTributeCard()) {
						...
						else if (c.baseTributes < tributes && c.baseTributes > highestTrib) {
							highestTrib = c.baseTributes;
						}
			...
			// If not, loop through again, and save all tribute monsters with the previously found tribute cost (that we know exists for at least 1 monster)
				...
				if (highestTrib > -1) {
					for (DuelistCard c : DuelistMod.myCards) {
						if (c.hasTag(Tags.MONSTER) && c.isTributeCard() && c.baseTributes == highestTrib) {
							possibleTributeMonsters.add((DuelistCard) c.makeStatEquivalentCopy());
						}
					}
				}
