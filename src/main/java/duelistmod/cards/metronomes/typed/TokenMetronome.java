...
public class TokenMetronome extends MetronomeCard 
{
    // TEXT DECLARATION
	...
	{
		ArrayList<DuelistCard> tmp = DuelistCardLibrary.getTokensForCombat();
		ArrayList<AbstractCard> ret = new ArrayList<>();
		while (tmp.size() > 0 && ret.size() < this.magicNumber) { ret.add(tmp.remove(AbstractDungeon.cardRandomRng.random(tmp.size() - 1))); }
		return ret;
	}
