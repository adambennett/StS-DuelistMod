...
{
    // TEXT DECLARATION
    public static final String ID = DuelistMod.makeID("FangCritias");
    private static final CardStrings cardStrings = CardCrawlGame.languagePack.getCardStrings(ID);
    public static final String IMG = DuelistMod.makeCardPath("FangCritias.png");
    ...
    public void use(AbstractPlayer p, AbstractMonster m) 
    {
    	...
    	for (AbstractCard c : p.exhaustPile.group)
    	{
    		...
    		{
    			choices.add((DuelistCard) c);
    		}
    	}
    	
    	if (choices.size() > 0) { this.addToBot(new FangCritiasAction(choices, 1, m)); }
    }

    // Which card to return when making a copy of this card.
    @Override
    public AbstractCard makeCopy() {
        return new FangCritias();
    }
