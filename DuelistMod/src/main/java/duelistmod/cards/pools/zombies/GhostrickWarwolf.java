...
public class GhostrickWarwolf extends DuelistCard 
{
    // TEXT DECLARATION
    ...
    public void use(AbstractPlayer p, AbstractMonster m) 
    {
    	tribute();
    	attack(m);
    	ArrayList<AbstractCard> rands = new ArrayList<>();
    	ArrayList<AbstractCard> choices = new ArrayList<>();
    	for (AbstractCard c : p.hand.group) { if (DuelistCard.allowResummonsWithExtraChecks(c)) {
    			rands.add(c);
    		}
    	}
    	
    	while (choices.size() < this.magicNumber && rands.size() > 0)
    	{
    		int inde = AbstractDungeon.cardRandomRng.random(rands.size() - 1);
    		choices.add(rands.get(inde));
    		rands.remove(inde);
    	}
    	
    	if (choices.size() > 0) { this.addToBot(new CardSelectScreenResummonAction(choices, 1, m)); }
