...
public class BoosterPackSelectScreen extends GridCardSelectScreen implements ScrollBarListener
{
    ...
    public BoosterPackSelectScreen(boolean allowUpgrades) {
        this.grabStartY = 0.0f;
        this.currentDiffY = 0.0f;
        this.selectedCards = new ArrayList<AbstractCard>();
        ...
        this.confirmButton = new GridSelectConfirmButton(BoosterPackSelectScreen.TEXT[0]);
        ...
        BoosterPackSelectScreen.drawStartX = (float)Settings.WIDTH;
        BoosterPackSelectScreen.drawStartX -= 5.0f * AbstractCard.IMG_WIDTH * 0.75f;
        BoosterPackSelectScreen.drawStartX -= 4.0f * Settings.CARD_VIEW_PAD_X;
        BoosterPackSelectScreen.drawStartX /= 2.0f;
        BoosterPackSelectScreen.drawStartX += AbstractCard.IMG_WIDTH * 0.75f / 2.0f;
        BoosterPackSelectScreen.padX = AbstractCard.IMG_WIDTH * 0.75f + Settings.CARD_VIEW_PAD_X;
        BoosterPackSelectScreen.padY = AbstractCard.IMG_HEIGHT * 0.75f + Settings.CARD_VIEW_PAD_Y;
                ...
                if (!this.selectedCards.contains(this.hoveredCard) && this.selectedCards.size() < this.numCards) {
                    ...
                    if (this.numCards == this.cardSelectAmount) {
                        if (this.forUpgrade) {
                            ...
                            this.selectedCards.clear();
                            AbstractDungeon.overlayMenu.cancelButton.show(BoosterPackSelectScreen.TEXT[1]);
                            this.confirmButton.show();
                            this.confirmButton.isDisabled = false;
                            this.lastTip = this.tipMsg;
                            this.tipMsg = BoosterPackSelectScreen.TEXT[2];
                            return;
                        }
                        ...
                        if (!this.anyNumber) {
                            ...
                            if (AbstractDungeon.screen != AbstractDungeon.CurrentScreen.SHOP) {
                                ...
                                AbstractDungeon.overlayMenu.cancelButton.show(BoosterPackSelectScreen.TEXT[3]);
                            }
                            for (final AbstractCard c : this.selectedCards) {
                                c.stopGlowing();
                            }
    ...
    private void updateCardPositionsAndHoverLogic() {
        if (this.isJustForConfirming && this.targetGroup.size() <= 4) {
            switch (this.targetGroup.size()) {
                ...
                case 2: {
                    this.targetGroup.group.get(0).current_x = Settings.WIDTH / 2.0f - BoosterPackSelectScreen.padX / 2.0f;
                    this.targetGroup.group.get(0).target_x = Settings.WIDTH / 2.0f - BoosterPackSelectScreen.padX / 2.0f;
                    this.targetGroup.group.get(1).current_x = Settings.WIDTH / 2.0f + BoosterPackSelectScreen.padX / 2.0f;
                    this.targetGroup.group.get(1).target_x = Settings.WIDTH / 2.0f + BoosterPackSelectScreen.padX / 2.0f;
                    break;
                }
                case 3: {
                    this.targetGroup.group.get(0).current_x = BoosterPackSelectScreen.drawStartX + BoosterPackSelectScreen.padX;
                    this.targetGroup.group.get(1).current_x = BoosterPackSelectScreen.drawStartX + BoosterPackSelectScreen.padX * 2.0f;
                    this.targetGroup.group.get(2).current_x = BoosterPackSelectScreen.drawStartX + BoosterPackSelectScreen.padX * 3.0f;
                    this.targetGroup.group.get(0).target_x = BoosterPackSelectScreen.drawStartX + BoosterPackSelectScreen.padX;
                    this.targetGroup.group.get(1).target_x = BoosterPackSelectScreen.drawStartX + BoosterPackSelectScreen.padX * 2.0f;
                    this.targetGroup.group.get(2).target_x = BoosterPackSelectScreen.drawStartX + BoosterPackSelectScreen.padX * 3.0f;
                    break;
                }
                case 4: {
                    this.targetGroup.group.get(0).current_x = Settings.WIDTH / 2.0f - BoosterPackSelectScreen.padX / 2.0f - BoosterPackSelectScreen.padX;
                    this.targetGroup.group.get(0).target_x = Settings.WIDTH / 2.0f - BoosterPackSelectScreen.padX / 2.0f - BoosterPackSelectScreen.padX;
                    this.targetGroup.group.get(1).current_x = Settings.WIDTH / 2.0f - BoosterPackSelectScreen.padX / 2.0f;
                    this.targetGroup.group.get(1).target_x = Settings.WIDTH / 2.0f - BoosterPackSelectScreen.padX / 2.0f;
                    this.targetGroup.group.get(2).current_x = Settings.WIDTH / 2.0f + BoosterPackSelectScreen.padX / 2.0f;
                    this.targetGroup.group.get(2).target_x = Settings.WIDTH / 2.0f + BoosterPackSelectScreen.padX / 2.0f;
                    this.targetGroup.group.get(3).current_x = Settings.WIDTH / 2.0f + BoosterPackSelectScreen.padX / 2.0f + BoosterPackSelectScreen.padX;
                    this.targetGroup.group.get(3).target_x = Settings.WIDTH / 2.0f + BoosterPackSelectScreen.padX / 2.0f + BoosterPackSelectScreen.padX;
                    break;
                }
            }
            final ArrayList<AbstractCard> c2 = this.targetGroup.group;
            for (int i = 0; i < c2.size(); ++i) {
                c2.get(i).target_y = BoosterPackSelectScreen.drawStartY + this.currentDiffY;
        ...
        for (int j = 0; j < cards.size(); ++j) {
            ...
            cards.get(j).target_x = BoosterPackSelectScreen.drawStartX + mod * BoosterPackSelectScreen.padX;
            cards.get(j).target_y = BoosterPackSelectScreen.drawStartY + this.currentDiffY - lineNum * BoosterPackSelectScreen.padY;
    ...
    public void open(final CardGroup group, final int numCards, final String msg) {
        ...
        this.confirmButton.show();
        this.confirmButton.updateText(BoosterPackSelectScreen.TEXT[0]);
        this.confirmButton.isDisabled = false;
    }
    ...
    private void callOnOpen() {
        ...
        if (this.targetGroup.group.size() <= 5) {
            BoosterPackSelectScreen.drawStartY = Settings.HEIGHT * 0.5f;
        }
        else {
            BoosterPackSelectScreen.drawStartY = Settings.HEIGHT * 0.66f;
        }
    }
    ...
    private void calculateScrollBounds() {
        ...
        if (this.targetGroup.size() > 10) {
            ...
            this.scrollUpperBound = Settings.DEFAULT_SCROLL_LIMIT + scrollTmp * BoosterPackSelectScreen.padY;
        }
        else {
            this.scrollUpperBound = Settings.DEFAULT_SCROLL_LIMIT;
        }
    ...
    private void hideCards() {
        ...
        for (int i = 0; i < cards.size(); ++i) {
            ...
            cards.get(i).current_x = BoosterPackSelectScreen.drawStartX + mod * BoosterPackSelectScreen.padX;
            cards.get(i).current_y = BoosterPackSelectScreen.drawStartY + this.currentDiffY - lineNum * BoosterPackSelectScreen.padY - MathUtils.random(100.0f * Settings.scale, 200.0f * Settings.scale);
    ...
    public void cancelUpgrade() {
        ...
        for (int i = 0; i < cards.size(); ++i) {
            ...
            cards.get(i).current_x = BoosterPackSelectScreen.drawStartX + mod * BoosterPackSelectScreen.padX;
            cards.get(i).current_y = BoosterPackSelectScreen.drawStartY + this.currentDiffY - lineNum * BoosterPackSelectScreen.padY;
        }
    ...
    private boolean shouldShowScrollBar() {
        return !this.confirmScreenUp && this.scrollUpperBound > BoosterPackSelectScreen.SCROLL_BAR_THRESHOLD && !PeekButton.isPeeking;
    }
    
    static {
        uiStrings = CardCrawlGame.languagePack.getUIString("GridCardSelectScreen");
        TEXT = BoosterPackSelectScreen.uiStrings.TEXT;
