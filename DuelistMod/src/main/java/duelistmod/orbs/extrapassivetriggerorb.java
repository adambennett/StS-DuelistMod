...
public class extrapassivetriggerorb extends DuelistOrb
{
	...
	public void updateDescription()
	{
		...
		{
			if (this.evokeAmount != 1) { this.description = DESC[0] + this.passiveAmount + DESC[4] + this.evokeAmount + DESC[2]; }
	...
	public void onEvoke()
	{		
		ArrayList<AbstractCard> cardsInDeck = new ArrayList<AbstractCard>();
		for (AbstractCard c : AbstractDungeon.player.drawPile.group) { if (c.cost > 0) { cardsInDeck.add(c); } }
		AbstractCard randomChoice = cardsInDeck.get(AbstractDungeon.cardRandomRng.random(cardsInDeck.size() - 1));
		randomChoice.modifyCostForCombat(-randomChoice.cost);
		randomChoice.isCostModified = true;
	}

	@Override
	public void onStartOfTurn()
	{
		if (this.passiveAmount > 0) { for (int i = 0; i < this.passiveAmount; i++) { triggerPassiveEffect(); }}
	}
	...
	public void triggerPassiveEffect()
	{
		if (AbstractDungeon.player.hasOrb())
		{
			...
			AbstractOrb rand = AbstractDungeon.player.orbs.get(AbstractDungeon.cardRandomRng.random(AbstractDungeon.player.filledOrbCount() - 1));
			if (!(rand instanceof EmptyOrbSlot)) { rand.onStartOfTurn(); }
			if (DuelistMod.debug) { DuelistMod.logger.info("DuelistCrystal triggered " + rand.name + "'s onStartOfTurn()"); }
		}
	}
	...
	{
		return new extrapassivetriggerorb();
	}
