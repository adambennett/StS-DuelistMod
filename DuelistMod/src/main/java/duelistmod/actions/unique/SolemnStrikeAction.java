	...
	public void update()
	{
		...
		if (this.duration == Settings.ACTION_DUR_MED)
		{
			...
			for (int i = 0; i < 2; i++)
			{
				...
				Util.log("theDuelist:SolemnStrikeAction:update() ---> powerCard.powerToApply was set to: " + power.name + ", with amount: " + power.amount);
		...
		if ((AbstractDungeon.gridSelectScreen.selectedCards.size() != 0))
		{
			for (AbstractCard c : AbstractDungeon.gridSelectScreen.selectedCards)
			{
				...
				if (c instanceof BuffCard)
				{
					...
					Util.log("theDuelist:SolemnStrikeAction:update() ---> bC.powerToApply was set to: " + powerMap.get(bC.uuid).name + ", with amount: " + powerMap.get(bC.uuid).amount);
