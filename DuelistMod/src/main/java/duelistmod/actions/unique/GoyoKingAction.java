...
public class GoyoKingAction extends AbstractGameAction
{
    private static final float DURATION = 0.1f;

    public GoyoKingAction(AbstractMonster monster)
    {
        target = monster;
    ...
    public void update()
    {
        if (duration == DURATION && target != null) 
        {
            if (target.isDying || target.currentHealth <= 0) 
            {
            	...
            	{ 
            		if (c.hasTag(Tags.WARRIOR) && c.canUpgrade()) { spells.add(c); }
            		else { deck.add(c); }
            	}
            	...
            	if (spells.size() > 0)
            	{
	            	...
	            	Util.log("Goyo King upgraded: " + randSpell.originalName);
	            	deck.add(randSpell);
            	...
            	{
            		Util.log("Goyo King found no Warriors in your deck");
            	}
