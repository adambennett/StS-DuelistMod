...
public class SolidarityExhaustPower extends DuelistPower
{	
	public AbstractCreature source;

    public static final String POWER_ID = DuelistMod.makeID("SolidarityExhaustPower");
    private static final PowerStrings powerStrings = CardCrawlGame.languagePack.getPowerStrings(POWER_ID);
    public static final String NAME = powerStrings.NAME;
    public static final String[] DESCRIPTIONS = powerStrings.DESCRIPTIONS;
    public static final String IMG = DuelistMod.makePowerPath("SolidarityExhaustPower.png");
	
    private CardTags currentType = Tags.ALL;		// Tags.ALL is a null flag in this context
    private boolean noTyped = false;
    
	public SolidarityExhaustPower() 
	{ 
		this.name = NAME;
        this.ID = POWER_ID;
	...
	public CardTags solidarity()
	{
		...
		if (AbstractDungeon.player.exhaustPile.group.size() < 1) {  updateDescription(); this.noTyped = true; return Tags.ALL; }
		...
		for (AbstractCard c : AbstractDungeon.player.exhaustPile.group) { if (c.hasTag(Tags.MONSTER) && c instanceof DuelistCard) { typedMonsters.add((DuelistCard) c); }}
