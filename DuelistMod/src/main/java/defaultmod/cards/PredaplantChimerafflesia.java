...
{
    // TEXT DECLARATION
    public static final String ID = DefaultMod.makeID("PredaplantChimerafflesia");
    private static final CardStrings cardStrings = CardCrawlGame.languagePack.getCardStrings(ID);
    public static final String IMG = DefaultMod.makePath(DefaultMod.PREDAPLANT_CHIMERAFFLESIA);
    ...
    public void use(AbstractPlayer p, AbstractMonster m) 
    {
    	...
    	if (tributeList.size() > 0)
    	{
    		for (DuelistCard c : tributeList)
    		{
    			...
    			{
    				if (DefaultMod.debug) { System.out.println("tributed something non predplant: " + c.originalName); }
    			}
    		}
    	}
    }
