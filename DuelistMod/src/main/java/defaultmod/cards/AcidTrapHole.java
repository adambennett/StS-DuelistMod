...
{
    // TEXT DECLARATION
    public static final String ID = DefaultMod.makeID("AcidTrapHole");
    private static final CardStrings cardStrings = CardCrawlGame.languagePack.getCardStrings(ID);
    public static final String IMG = DefaultMod.makePath(DefaultMod.ACID_TRAP);
    ...
    public void use(AbstractPlayer p, AbstractMonster m) 
    {
    	...
    	for (AbstractCard c : drawPile)
    	{
    		if (c.hasTag(DefaultMod.MONSTER))
    		{
    			...
    			if (DefaultMod.debug) { System.out.println("theDuelist:AcidTrapHole:use() ---> added " + c.originalName + " to drawPileMonsters"); }
    		}
    	}
    	...
    	if (!(drawPileMonsters.size() > this.magicNumber))
    	{
    		...
    		if (DefaultMod.debug) { System.out.println("theDuelist:AcidTrapHole:use() ---> monstersToPull set to drawPileMonsters.size()"); }
    	}
    	...
    	for (int i = 0; i < monstersToPull; i++)
    	{
    		...
    		if (DefaultMod.debug) { System.out.println("theDuelist:AcidTrapHole:use() ---> added " + random.originalName + " to randomMonsters"); }
    	}
    	...
    	for (int i = 0; i < randomMonsters.size(); i++)
    	{
			...
			if (DefaultMod.debug) { System.out.println("theDuelist:AcidTrapHole:use() ---> added " + randomMonsters.get(i).originalName + " to toDiscard :: damageTotal increased by: " + randomMonsters.get(i).baseDamage); }
    	}

    	for (AbstractCard c : toDiscard)
    	{
    		AbstractDungeon.player.drawPile.moveToDiscardPile(c);
    		if (DefaultMod.debug) { System.out.println("theDuelist:AcidTrapHole:use() ---> moved " + c.originalName + " to discard pile"); }
