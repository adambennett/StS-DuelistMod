...
public class MillenniumEffect
{
	...
	public static void runMillenniumEffect(int extra)
	{
		...
		{
			if (DefaultMod.fullDebug) { if (AbstractPlayer.customMods.size() < 1 && !DefaultMod.challengeMode) { runSpecialEffect(35 + extra); } else if (!DefaultMod.challengeMode) { DuelistCard.powerSummon(AbstractDungeon.player, 35, "Puzzle Token", false); } else { DuelistCard.summon(AbstractDungeon.player, 2, new ExplosiveToken("Exploding Token")); }}
			else
			{
				// Normal Runs
				if (AbstractPlayer.customMods.size() < 1 && !DefaultMod.challengeMode) { runSpecialEffect(extra); }
				
				// Custom Runs & No Challenge Mode
				else if (!DefaultMod.challengeMode) { DuelistCard.powerSummon(AbstractDungeon.player, 2 + extra, "Puzzle Token", false); }
